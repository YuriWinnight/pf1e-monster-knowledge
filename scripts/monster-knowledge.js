...
function registerSettings() {
  ...
  game.settings.register(MODULE_ID, "hideSharedFeatureHiddenText", {
    name: game.i18n.localize("PF1MK.Settings.HideSharedFeatureHiddenText.Name"),
    hint: game.i18n.localize("PF1MK.Settings.HideSharedFeatureHiddenText.Hint"),
    scope: "world",
    config: true,
    type: Boolean,
    default: true
...
async function openSharedFeatureDialog(entry) {
  ...
  const visibleDescription = hideHiddenText
    ...
    : exposeHiddenFeatureHtml(enriched || sourceDescription);
  const viewerClass = hideHiddenText ? "pf1mk-feature-viewer" : "pf1mk-feature-viewer pf1mk-reveal-hidden-feature-text";
  const content = `
...
function getItemDescriptionHtml(item) {
  ...
  const addHidden = (value) => {
    ...
    const text = String(value);
    if (!text.trim()) return;
    add(`<section class="secret pf1mk-feature-hidden-description">${text}</section>`);
  };

  if (description && typeof description === "object") {
    for (const key of ["value", "short", "long", "identified", "unidentified", "public", "notes", "chat", "text"]) {
      add(description[key]);
    }
    for (const key of ["private", "gm", "gmNotes", "gmnotes", "gmNote"]) {
      addHidden(description[key]);
    }
  } else {
    add(description);
  }
  ...
  addHidden(item?.system?.effectNotes);
  addHidden(item?.system?.details);
  addHidden(item?.system?.gmNotes);
  addHidden(item?.system?.gmnotes);

  return parts.join("\n");
}

function hideHiddenFeatureHtml(html) {
  const raw = String(html ?? "");
  if (!raw) return "";
  if (typeof document === "undefined") {
    return raw
      .replace(/<([a-z][\w:-]*)\b(?=[^>]*(?:class=["'][^"']*(?:secret|gm-only|gmOnly|gm-note|gmnotes)[^"']*["']|hidden\b|data-secret\b|data-visibility\b))[^>]*>[\s\S]*?<\/\1>/gi, "")
      .replace(/<([a-z][\w:-]*)\b(?=[^>]*style=["'][^"']*(?:display\s*:\s*none|visibility\s*:\s*hidden|opacity\s*:\s*0)[^"']*["'])[^>]*>[\s\S]*?<\/\1>/gi, "")
      .replace(/<[^>]*(?:hidden\b|data-secret\b|data-visibility\b)[^>]*\/?>/gi, "");
  }
  ...
  wrapper.querySelectorAll(hiddenSelector).forEach((element) => element.remove());
  wrapper.querySelectorAll("*").forEach((element) => {
    const styleText = element.getAttribute("style") || "";
    if (/display\s*:\s*none/i.test(styleText) || /visibility\s*:\s*hidden/i.test(styleText) || /opacity\s*:\s*0/i.test(styleText)) {
      element.remove();
    }
  });
...
function exposeHiddenFeatureHtml(html) {
  ...
  if (typeof document === "undefined") {
    return raw
      .replace(/\sclass=(["'])(.*?)\1/gi, (_match, quote, classes) => {
        const visibleClasses = String(classes).split(/\s+/).filter((className) => !["secret", "gm-only", "gmOnly", "gm-note", "gmnotes"].includes(className));
        return visibleClasses.length ? ` class=${quote}${visibleClasses.join(" ")}${quote}` : "";
      })
      .replace(/\s+(hidden|data-secret|data-visibility)(=(["']).*?\3|=[^\s>]+)?/gi, "")
